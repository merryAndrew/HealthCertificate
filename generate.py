...
def build_card(issue, style='A'):
    ...
    date_display = format_date(date_match.group(1).strip()) if date_match else '未选择日期 (有效期一年)'
    ...
    if style == 'A':
        return f'''
<div class="cert-wrapper" data-title="{name}" data-issue-number="{issue['number']}" data-card-type="A" style="position: relative; padding-bottom: 60px;">
    <div class="cert-module top-card">
        <div class="top-title" id="card1Title_{issue['number']}">{card1_title}</div>
        <div class="top-content">
            <div class="text-container">
                <div class="info-line"><span style="font-weight: bold;">姓 名</span><span>∶</span><span id="name_{issue['number']}">{name}</span></div>
                <div class="info-line"><span style="font-weight: bold;">性 别</span><span>∶</span><span id="gender_{issue['number']}">{gender}</span></div>
                <div class="id-group">
                    <div class="info-line"><span style="font-weight: bold;">身份证号码</span><span>∶</span><span id="id_{issue['number']}">{id_num}</span></div>
                    <div class="info-line">(或其它有效证明)</div>
                </div>
                <div class="info-line"><span style="font-weight: bold;">体检单位</span><span>∶</span><span id="hospital_{issue['number']}">{hospital}</span></div>
                <div class="info-line last-line"><span style="font-weight: bold;">体检日期</span><span>∶</span><span id="date_{issue['number']}">{date_display}</span></div>
            </div>
            <div class="photo">
                <div class="seal-container"><img class="seal-img" src="https://cdn.jsdelivr.net/gh/merryAndrew/imge@main/than.png" alt="印章"></div>
                <img src="{img_url}" alt="持证人照片">
            </div>
        </div>
    </div>
    <div class="cert-module middle-card">
        <div class="middle-line" id="card2Text_{issue['number']}">{card2_text}</div>
        <div class="middle-line">健康证明</div>
    </div>
    <div class="bottom-card">
        <div class="qrcode-area">
            <div class="qrcode-title">防伪标识二维码</div>
            <div class="qrcode-img"><img src="data:image/png;base64,{qr_base64}" alt="二维码"></div>
        </div>
        <div class="notice">
            <div class="notice-title">关于申请实体证明通知：</div>
            <div class="notice-content" id="card3Text_{issue['number']}">{card3_text}</div>
        </div>
    </div>
    <div class="button-group" id="buttonGroup_{issue['number']}" style="position: absolute; bottom: 10px; right: 12px; display: flex; gap: 8px; z-index: 30;">
    ...
    <div class="edit-status" id="editStatus_{issue['number']}" style="position:absolute; bottom:45px; right:12px; font-size:12px; z-index:20; display:none;"></div>
</div>
'''
    # ===== B版 =====
    elif style == 'B':
        return f'''
<div class="cert-wrapper" data-title="{name}" data-issue-number="{issue['number']}" data-card-type="B" style="position: relative; padding-bottom: 60px;">
    <div class="cert-module top-card">
        <div class="top-title" id="card1Title_{issue['number']}">{card1_title}</div>
        <div class="top-content">
            <div class="text-container">
                <div class="info-line"><span class="label">姓 名</span><span class="colon">∶</span><span class="content" id="name_{issue['number']}">{name}</span></div>
                <div class="info-line"><span class="label">性 别</span><span class="colon">∶</span><span class="content" id="gender_{issue['number']}">{gender}</span></div>
                <div class="id-group">
                    <div class="info-line"><span class="label">身份证号码</span><span class="colon">∶</span><span class="content" id="id_{issue['number']}">{id_num}</span></div>
                    <div class="info-line">(或其它有效证明)</div>
                </div>
                <div class="info-line"><span class="label">体检单位</span><span class="colon">∶</span><span class="content" id="hospital_{issue['number']}">{hospital}</span></div>
                <div class="info-line last-line"><span class="label">体检日期</span><span class="colon">∶</span><span class="content" id="date_{issue['number']}">{date_display}</span></div>
            </div>
            <div class="photo"><img src="{img_url}" alt="持证人照片"></div>
        </div>
    </div>
    <div class="cert-module middle-card">
        <div class="middle-line" id="card2Text_{issue['number']}">{card2_text}</div>
    ...
    <div class="bottom-card">
        <div class="qrcode-area">
            <div class="qrcode-img"><img src="data:image/png;base64,{qr_base64}" alt="二维码"></div>
            <div class="qrcode-title" id="card3Text_{issue['number']}">{card3_text}</div>
        </div>
    </div>
    <div class="button-group" id="buttonGroup_{issue['number']}" style="position: absolute; bottom: 10px; right: 12px; display: flex; gap: 8px; z-index: 30;">
        <button class="edit-btn" id="editBtn_{issue['number']}" style="background: #2b6ef0; color:#fff; border:none; border-radius:20px; padding:6px 16px; font-size:13px; cursor:pointer;">编辑</button>
...
'''
    # ===== C版 =====
        ...
        return f'''
<div class="cert-wrapper" data-title="{name}" data-issue-number="{issue['number']}" data-card-type="C" style="position: relative; padding-bottom: 60px;">
    <div class="cert-module top-card">
        <div class="top-title" id="card1Title_{issue['number']}">{card1_title}</div>
        <div class="top-content">
            <div class="text-container">
...
for issue in issues:
    if 'pull_request' in issue:
        continue
    ca = build_card(issue, 'A')
    cb = build_card(issue, 'B')
    cc = build_card(issue, 'C')
    if ca:
        cards_A.append(ca)
    if cb:
        cards_B.append(cb)
...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .not-found {{ text-align: center; padding: 40px 20px; font-size: 18px; color: #666; background: #f8f8f8; border-radius: 12px; }}
        .admin-bar {{ background: #fff; padding: 12px 16px; border-radius: 12px; margin-bottom: 20px; display: flex; flex-wrap: wrap; align-items: center; gap: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.06); }}
...
<body>
    <div id="app">
        <div id="adminBar" style="display:none;" class="admin-bar">
            ...
            <div class="checkbox-container"><input type="checkbox" id="selectAll" /><label for="selectAll">全选</label></div>
            <button class="btn-danger" id="batchDeleteBtn">删除数据</button>
            <button class="btn-success" id="showAllBtn">显示全部</button>
            <span class="badge" id="batchStatus"></span>
        </div>
        <div id="cardsContainer">{''.join(all_cards)}</div>
    ...
    (function() {{
        ...
        // 过滤卡片
        function filterCards(decodedId) {{
            let found = false;
            wrappers.forEach(w => {{
        ...
        if (isAdmin) {{
            ...
            if (idParam) {{
                try {{ filterCards(decodeURIComponent(idParam)); }} catch(e) {{ filterCards(idParam); }}
            }} else {{
        ...
        if (batchDeleteBtn) {{
            batchDeleteBtn.addEventListener('click', function() {{
                const checked = document.querySelectorAll('.card-select:checked');
                if (checked.length === 0) {{ alert('请至少选择一张卡片'); return; }}
                const persons = new Set();
                checked.forEach(cb => persons.add(cb.dataset.title));
                if (!confirm(`确定要删除 ${{persons.size}} 个用户的所有数据吗？不可恢复！`)) return;
                let token = localStorage.getItem('github_token');
                if (!token) {{ token = prompt('请输入 GitHub Token:'); if (token) localStorage.setItem('github_token', token); }}
                if (!token) {{ batchStatus.textContent = '❌ 需要 Token'; batchStatus.style.color = '#e53e3e'; return; }}
                batchDeleteBtn.disabled = true;
                batchStatus.textContent = '⏳ 删除中...';
                batchStatus.style.color = '#2b6ef0';
                const repo = 'merryAndrew/HealthCertificate';
                let completed = 0, failed = 0, total = persons.size;
                persons.forEach(name => {{
                    ...
                    .then(res => {{ if (!res.ok) throw new Error('失败'); completed++; }})
                    .catch(() => failed++)
                    .finally(() => {{
        ...
        if (showAllBtn) {{
            showAllBtn.addEventListener('click', function() {{
                ...
                const persons = new Set();
                checked.forEach(cb => persons.add(cb.dataset.title));
                if (!confirm(`确定恢复 ${{persons.size}} 个用户的所有卡片吗？`)) return;
                let token = localStorage.getItem('github_token');
                if (!token) {{ token = prompt('请输入 GitHub Token:'); if (token) localStorage.setItem('github_token', token); }}
                if (!token) {{ batchStatus.textContent = '❌ 需要 Token'; batchStatus.style.color = '#e53e3e'; return; }}
                ...
                persons.forEach(name => {{
                    ...
                    fetch(getUrl, {{ headers: {{ 'Authorization': `Bearer ${{token}}`, 'Accept': 'application/vnd.github.v3+json' }} }})
                    .then(res => res.json())
                    .then(data => {{
                        let body = data.body || '';
                        body = body.replace(/\\n?隐藏卡片[：:][^\\n]*/, '');
        ...
        document.querySelectorAll('.edit-btn').forEach(btn => {{
            ...
            btn.addEventListener('click', function() {{
                ...
                if (isEditing) {{
                    ...
                    [nameEl, genderEl, idEl, hospitalEl, dateEl, card1TitleEl, card2TextEl, card3TextEl].forEach(el => {{
                        if (el) {{ el.contentEditable = false; el.classList.remove('editable-field'); }}
                    }});
                    if (genderEl) {{
                        const sel = genderEl.querySelector('.gender-select');
                        if (sel) genderEl.innerHTML = sel.value;
                    }}
                ...
                [nameEl, genderEl, idEl, hospitalEl, dateEl, card1TitleEl, card2TextEl, card3TextEl].forEach(el => {{
                    if (el) {{ el.contentEditable = true; el.classList.add('editable-field'); }}
                }});
                if (genderEl) {{
                    const current = genderEl.textContent.trim();
                    ...
                    genderEl.innerHTML = `<select class="gender-select" style="padding:2px 6px;border-radius:4px;border:1px solid #2b6ef0;font-size:13px;">
                        <option value="男" ${{current==='男'?'selected':''}}>男</option>
                        <option value="女" ${{current==='女'?'selected':''}}>女</option>
                    </select>`;
                }}
            ...
            if (saveBtn) {{
                saveBtn.addEventListener('click', function() {{
                    ...
                    let newGender = '';
                    const gs = genderEl ? genderEl.querySelector('.gender-select') : null;
                    newGender = gs ? gs.value : (genderEl ? genderEl.textContent.trim() : '男');
                    ...
                    let avatarField = (imgUrl && !imgUrl.includes('placeholder')) ? `\\n头像图片：${{imgUrl}}` : '';

                    let token = localStorage.getItem('github_token');
                    if (!token) {{ token = prompt('请输入 GitHub Token:'); if (token) localStorage.setItem('github_token', token); }}
                    if (!token) {{ editStatus.textContent = '❌ 需要 Token'; editStatus.className = 'edit-status error'; return; }}
                    ...
                    .then(() => {{
                        ...
                        if (gs) genderEl.innerHTML = gs.value;
                        btn.textContent = '编辑';
                        saveBtn.style.display = 'none';
                        cancelBtn.style.display = 'none';
                        setTimeout(() => {{ editStatus.textContent = '🔄 刷新查看更新'; }}, 3000);
                    }})
                    .catch(err => {{ editStatus.textContent = '❌ ' + err.message; editStatus.className = 'edit-status error'; }});
                }});
            }}
        }});
    }})();
...
<body>
    <div class="app-wrapper">{''.join(all_cards)}</div>
    <script>
        (function() {{
            const params = new URLSearchParams(window.location.search);
            ...
            if (id) {{
                ...
                wrappers.forEach(w => {{
                    ...
                    if (title === id) {{ w.style.display = 'block'; found = true; }} else {{ w.style.display = 'none'; }}
                }});
                if (!found) wrappers.forEach(w => w.style.display = 'block');
            }}
        }})();
    </script>
</body>
